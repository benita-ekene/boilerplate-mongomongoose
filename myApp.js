...
const queryChain = (done) => {
  ...
  Person.find({favoriteFoods: foodToSearch}, (err, foodFound) => {
    if(err)
    done(err)
    foodFound.sort({name: foodToSearch.favoriteFoods}).limit(2).select().exec((err, resultFound) => {
      if(err)
      done(err)
      done(null, resultFound)
    }) 
  })
